...
var insecureCreds = clientGrpc.credentials.createInsecure();

const echoProtoDef = protoLoader.loadSync(__dirname + '/../proto/echo_service.proto', options);
const EchoClient = clientGrpc.loadPackageDefinition(echoProtoDef).EchoService;
const echo_service = echoProtoDef.EchoService;

var StatusBuilder = clientGrpc.StatusBuilder;
var ListenerBuilder = clientGrpc.ListenerBuilder;
var InterceptingCall = clientGrpc.InterceptingCall;
var RequesterBuilder = clientGrpc.RequesterBuilder;
const Metadata = clientGrpc.Metadata;
...
describe('Client interceptors', function() {
        ...
        .withHalfClose(function(next) {
          ...
          if (cachedValue) {
            ...
            storedListener.onReceiveStatus(
              (new StatusBuilder()).withCode(clientGrpc.status.OK).build());
          } else {
                    ...
                    onReceiveStatus: function(status) {
                      ...
                      if (status.code !== clientGrpc.status.OK) {
                        ...
                        var new_status = (new StatusBuilder())
                          .withCode(clientGrpc.status.OK).build();
                        savedMessageNext(receivedMessage);
                        next(new_status);
                      }
                    }
  ...
  it('uses interceptors passed to the client constructor', function(done) {
    ...
    var constructor_interceptor_a = function(options, nextCall) {
      var outbound = (new RequesterBuilder())
        .withStart(function(metadata, listener, next) {
          registry.addCall('constructor_interceptor_a_echo');
          next(metadata, listener);
        }).build();
      return new InterceptingCall(nextCall(options), outbound);
    };
    var constructor_interceptor_b = function(options, nextCall) {
      var outbound = (new RequesterBuilder())
        .withStart(function(metadata, listener, next) {
          registry.addCall('constructor_interceptor_b_echoServerStream');
          next(metadata, listener);
        }).build();
      return new InterceptingCall(nextCall(options), outbound);
    };
  ...
  describe('trigger a stack of interceptors in nested order', function() {
    ...
    before(function() {
      var interceptor_a = function (options, nextCall) {
        ...
        return new clientGrpc.InterceptingCall(nextCall(options),
          outbound);
      };
      var interceptor_b = function (options, nextCall) {
